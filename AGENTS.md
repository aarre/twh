...
* `twh effort` collects effort ordering with pairwise prompts (transitive) and writes numeric `diff` UDA values (0 easiest, 10 hardest); `twh ondeck` only collects effort for moves tied within a dominance tier.
* `twh ondeck` tracks `diff` (pairwise effort ranking) alongside `imp`/`urg`/`opt_human` (legacy `opt` accepted)/`mode`/`criticality` and ranks moves by dominance tier before scoring ties; missing metadata, criticality, and effort are only collected for moves in dominance ties.
...
* `twh add` is an interactive flow that prompts for description, project, tags, due date, blocks, and metadata (imp/urg/opt_human/mode), then runs dominance sorting.
...
- Effort comparisons are cached to `~/.config/twh/effort.json` (override with `TWH_EFFORT_PATH`) so the workflow can resume after interruptions.
- `twh` delegates unknown commands (including no-arg invocation) to Taskwarrior; `list`, `reverse`, `tree`, `graph`, `simple`, `ondeck`, `defer`, `diagnose`, `option`, `calibrate`, `start`, `stop`, `time`, `dominance`, `criticality`, and `effort` are handled internally.
...
- `twh ondeck` inspects pending moves for dominance ordering first, then prompts for missing `imp`, `urg`, `opt_human` (legacy `opt` accepted), and `mode` only for moves tied within a dominance tier; it then collects missing `criticality` and `diff` (effort) via pairwise comparisons for tied tiers, auto-runs `twh option --apply` after metadata updates, and recommends the next move using dominance tiers before the scoring model; mode filters (`--mode`, `--strict-mode`) and dominance UDAs influence candidate selection, extra Taskwarrior filter tokens after `twh ondeck` scope the set, and filtered runs still apply the wizard for tied moves even when blocked.
...
- `twh start`/`twh stop` log time entries (uuid/description/project/tags/mode/start/end) to `twh-time.db` inside Taskwarrior's data directory (`data.location` in `~/.taskrc`, defaulting to `~/.task`), auto-stop other active moves on start, and `twh time` reports by task/project/tag/mode with day/week/month/year/range bucketing plus date filtering and record edits.
