...
`twh graph2` renders a Graphviz-based dependency graph when `--png` or `--svg`
is provided and falls back to an ASCII tree when Graphviz (`dot`) is not
available. Use `--rankdir` to change layout direction and `--edges` to print
the raw edge list.
